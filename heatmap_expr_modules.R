...
sampleInfo <- lapply(donorNames, function(d){
  ...
  color <- ontology$color_hex_triplet[match(sampleIds, ontology$id)]
  color <- sapply(color, function(c){
    if (nchar(c) == 5) {paste("#0", c, sep = "")} else {paste("#", c, sep = "")}
  })
  order <- ontology$graph_order[match(sampleIds, ontology$id)]
  data.frame(id = sampleIds, color, order, stringsAsFactors = FALSE)
})
...
lapply(braakNames, function(b){
  ...
  lapply(donorNames, function(d){
    ...
    colOrder <- rev(order(df$order))
    df <- df[colOrder, ]
    exprMat <- exprMat[, colOrder]
    
    colPal <- c("darkblue", "white", "darkred")
    rampcols <- colorRampPalette(colors = colPal, space="Lab")(100)
    heatmap.2(exprMat, col = rampcols, Rowv=FALSE, Colv=FALSE, scale = "none",
              trace = "none", dendrogram = "none", labRow = NA, labCol = NA, key = TRUE, 
              RowSideColors = colors, ColSideColors = df$color)
    title(paste0("Expression of modules in ", b, " and ", d))

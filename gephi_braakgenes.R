...
lapply(names(modules_braak), function(r){
  ...
  nodesAtt <- data.frame(membership = m$module[gene_idx], cor = cor, braak_gene = braak_gene)
  ...
  edges <- edges[abs(edges$coexpr) > 0.5, ] # 
  filter edges 

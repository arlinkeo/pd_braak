# Compare results based on different cell-types referecen (mean, svd, and pca)
...
# Barplot
# summaryCoef_pca <- readRDS("resources/diif_expr_EQ2/summaryCoef_pca.rds")
# summaryCoef_mean <- readRDS("resources/diif_expr_EQ2/summaryCoef_mean.rds")
# summaryCoef_svd <- readRDS("resources/diif_expr_EQ2/summaryCoef_svd.rds")
...
brg_neg <- braakGenes$entrez_id[braakGenes$r<0]
brg_pos <- braakGenes$entrez_id[braakGenes$r>0]
...
arr <- simplify2array(lapply(summTables_pca, as.matrix))
fc <- arr[celltypes$Neurons, "Estimate",]
fc <- fc[order(fc[,5]),]
df <- melt(fc)
...
degLists <- t(sapply(summTables_pca, function(t){
...
summaryCoef <- lapply(summaryCoef_pca, simplify2array)

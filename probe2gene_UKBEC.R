...
probeSelection <- collapseRows(exprConcat, expr.map$tID, expr.map$exprID, method = "maxRowVariance", connectivityBasedCollapsing = TRUE)

# Select probes for each data matrix
probeNames <- names(probeSelection$selectedRow)[probeSelection$selectedRow] # selected probes (exprID)
...
mappedGenes<- apply(t.map, 1, function(t){ # For each gene, check overlap in name symbols
  ...
  overlap <- intersect(genes, AHBAgenes) # should be 1 or no gene
  if (length(overlap) != 1) NA else overlap
})
probe2gene.map <- expr.map[probeSelection$selectedRow, c("exprID", "tID")] # selected probe for each gene
probe2gene.map <- cbind(probe2gene.map, t.map$Gene, t.map$Symbol.NA31)
probe2gene.map$AHBA <- mappedGenes

save(probe2gene.map, file = "../UKBEC_RData/probe2gene.map.RData")

# exprID <- probe2gene$exprID[match(rownames(t.map), probe2gene$tID)]
# t.map2 <- cbind(t.map, AHBA = mappedGenes, exprID = expr.map$exprID[probeSelection$selectedRow])
